...
import { clearLastLauncherIssue, readLastLauncherIssue, recordLauncherIssue, type LauncherIssue } from "./launcherIssueStore";
...
const LAUNCHER_REPORTS_API_URL = "https://api.nordiee.com/api/v1/launcher/reports";
...
export default function App() {
  ...
  useEffect(() => {
    const captureError = (event: ErrorEvent) => { recordLauncherIssue(event.error ?? event.message); };
    const captureRejection = (event: PromiseRejectionEvent) => { recordLauncherIssue(event.reason); };
...
function Launcher({ session, onRefreshSession, onSwitchAccount, onLogOff, onRemoveAccount }: { session: Session; onRefreshSession: () => Promise<string | null>; onSwitchAccount: () => void; onLogOff: () => void; onRemoveAccount: (account: SavedAccount) => Promise<void> }) {
  ...
  const [performanceMode, setPerformanceMode] = useState(() => localStorage.getItem("nordiee.performanceMode") !== "false");
  ...
  useEffect(() => {
    if (reduceMotion || (performanceMode && runningGameIds.length > 0)) document.documentElement.dataset.reduceMotion = "true";
    else delete document.documentElement.dataset.reduceMotion;
  }, [performanceMode, reduceMotion, runningGameIds.length]);
  useEffect(() => {
    let active = true;
    ...
    void checkHealth();
    const interval = window.setInterval(() => void checkHealth(), performanceMode && runningGameIds.length > 0 ? 300_000 : 60_000);
    return () => { active = false; window.clearInterval(interval); };
  }, [performanceMode, runningGameIds.length]);
  useEffect(() => {
    let active = true;
    ...
    const interval = window.setInterval(() => void checkFriendRequests(), performanceMode && runningGameIds.length > 0 ? 300_000 : 60_000);
    return () => { active = false; window.clearInterval(interval); };
  }, [addNotification, performanceMode, runningGameIds.length, session.accessToken]);
  useEffect(() => {
  ...
  const savePerformanceMode = (enabled: boolean) => { setPerformanceMode(enabled); localStorage.setItem("nordiee.performanceMode", String(enabled)); };
  const saveStartupView = (startupView: View) => localStorage.setItem("nordiee.startupView", startupView);
...
function Friends({ accessToken, onRefreshSession, onBack }: { accessToken: string; onRefreshSession: () => Promise<string | null>; onBack: () => void }) {
  ...
  const friendNotesKey = `nordiee.friendNotes.${(activeAccountEmail() ?? "guest").trim().toLocaleLowerCase()}`;
  const [friendNotes, setFriendNotes] = useState<Record<string, string>>(() => {
    try {
      const saved = JSON.parse(localStorage.getItem(friendNotesKey) ?? "{}");
  ...
  const [profilePreview, setProfilePreview] = useState<FriendProfilePreview | null>(null);
  const [profilePreviewState, setProfilePreviewState] = useState<"idle" | "loading" | "error">("idle");
  ...
  const copyFriendCode = async () => {
    ...
    } catch { setMessage("Could not copy Friend Code. Select it manually."); }
  };
  const openFriendProfile = async (friend: FriendProfile) => {
    setProfilePreview(null);
    setProfilePreviewState("loading");
    try {
      const response = await fetch(`${PROFILE_API_URL}/${encodeURIComponent(friend.username)}`, { headers: { Authorization: `Bearer ${accessToken}` } });
      if (!response.ok) throw new Error();
      setProfilePreview(await response.json() as FriendProfilePreview);
      setProfilePreviewState("idle");
    } catch { setProfilePreviewState("error"); }
  };
  ...
  const beginFriendNote = (friendId: string) => {
    setEditingFriendNoteId(friendId);
    setFriendNoteDraft(friendNotes[friendId] ?? "");
  };
  const saveFriendNote = (friendId: string) => {
    const note = friendNoteDraft.trim().slice(0, 120);
    setFriendNotes((current) => {
      const next = { ...current };
      if (note) next[friendId] = note;
      else delete next[friendId];
      localStorage.setItem(friendNotesKey, JSON.stringify(next));
      return next;
    });
    setEditingFriendNoteId(null);
    setFriendNoteDraft("");
  };
  ...
  return <main className="friends-page">
    ...
    </section>{profilePreviewState !== "idle" || profilePreview ? <div className="friend-profile-overlay" role="presentation" onMouseDown={() => { setProfilePreview(null); setProfilePreviewState("idle"); }}><section className="friend-profile-dialog" role="dialog" aria-modal="true" aria-label="Friend profile" onMouseDown={(event) => event.stopPropagation()}>{profilePreviewState === "loading" ? <p>Loading profile...</p> : profilePreviewState === "error" ? <><h2>Profile unavailable</h2><p>This profile is private or could not be loaded.</p></> : profilePreview ? <><header><span className="friend-profile-avatar" style={profilePreview.avatarUrl ? { backgroundImage: `url(${profilePreview.avatarUrl})` } : undefined}>{profilePreview.avatarUrl ? <span className="sr-only">{profilePreview.username}</span> : profilePreview.username[0]?.toUpperCase()}</span><div><p className="eyebrow">PLAYER PROFILE</p><h2>{profilePreview.username}</h2><p>{profilePreview.presenceStatus.toLocaleLowerCase()}</p></div></header><p className="friend-profile-bio">{profilePreview.bio || "No bio yet."}</p></> : null}<button className="text-button" type="button" autoFocus onClick={() => { setProfilePreview(null); setProfilePreviewState("idle"); }}>Close</button></section></div> : null}
  </main>;
}
...
function StartupViewSettings({ onChange }: { onChange: (view: View) => void }) {
  const [startupView, setStartupView] = useState<View>(readStartupView);
  const save = (next: View) => { setStartupView(next); onChange(next); };
  return <article className="panel"><p className="panel-label">STARTUP</p><div className="panel-heading"><div><h3>Open on launch</h3><p>Choose the first Nordiee page shown after you sign in.</p></div><label><span className="sr-only">Startup page</span><select className="select-button" value={startupView} onChange={(event) => save(event.target.value as View)}><option value="Home">Home</option><option value="Library">Library</option><option value="Downloads">Downloads</option><option value="Friends">Friends</option></select></label></div></article>;
}

function PerformanceSettings({ enabled, onChange }: { enabled: boolean; onChange: (enabled: boolean) => void }) {
  return <article className="panel"><p className="panel-label">PERFORMANCE</p><div className="panel-heading"><div><h3>Performance Mode</h3><p>When a Nordiee game runs, reduce motion and check background services less often.</p></div><button className={enabled ? "toggle enabled" : "toggle"} type="button" aria-pressed={enabled} aria-label="Performance Mode" onClick={() => onChange(!enabled)} /></div></article>;
}
...
function Settings({ accessToken, downloadSchedule, launcherNotificationsEnabled, manualUpdateState, onDownloadScheduleChange, onLauncherNotificationsEnabledChange, onManualUpdateCheck, onReduceMotionChange, onRefreshSession, pauseDownloadsWhilePlaying, reduceMotion, onPauseDownloadsWhilePlayingChange }: { accessToken: string; downloadSchedule: DownloadSchedule; launcherNotificationsEnabled: boolean; manualUpdateState: ManualUpdateState; onDownloadScheduleChange: (schedule: DownloadSchedule) => void; onLauncherNotificationsEnabledChange: (enabled: boolean) => void; onManualUpdateCheck: () => void; onReduceMotionChange: (enabled: boolean) => void; onRefreshSession: () => Promise<string | null>; pauseDownloadsWhilePlaying: boolean; reduceMotion: boolean; onPauseDownloadsWhilePlayingChange: (enabled: boolean) => void }) {
  ...
  const [lastIssue, setLastIssue] = useState<LauncherIssue | null>(readLastLauncherIssue);
  const [sendingReport, setSendingReport] = useState<"diagnostic" | "crash" | null>(null);
  const [reportMessage, setReportMessage] = useState<{ message: string; error: boolean } | null>(null);
  ...
  const sendSupportReport = async (kind: "DIAGNOSTIC" | "CRASH") => {
    setReportMessage(null);
    setSendingReport(kind === "DIAGNOSTIC" ? "diagnostic" : "crash");
    const issue = readLastLauncherIssue();
    const context = `API: ${diagnosticLabel(diagnostics.api)}; Downloads: ${diagnosticLabel(diagnostics.downloads)}; Storage: ${diagnosticLabel(diagnostics.storage)}`;
    const payload = {
      kind,
      launcherVersion: await invoke<string>("launcher_version"),
      summary: kind === "CRASH" ? issue?.message ?? "No local launcher error is available." : "User submitted a diagnostic report.",
      context: kind === "CRASH" ? `${context}; ${issue?.details ?? "No local error details are available."}`.slice(0, 600) : context,
    };
    const submit = (token: string) => fetch(LAUNCHER_REPORTS_API_URL, { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    try {
      let response = await submit(accessToken);
      if (response.status === 401 || response.status === 403) {
        const refreshedToken = await onRefreshSession();
        if (refreshedToken) response = await submit(refreshedToken);
      }
      if (!response.ok) throw new Error();
      if (kind === "CRASH") { clearLastLauncherIssue(); setLastIssue(null); }
      setReportMessage({ message: kind === "CRASH" ? "Launcher error report sent." : "Diagnostic report sent.", error: false });
    } catch { setReportMessage({ message: "We could not send this report. Export it locally and try again later.", error: true }); }
