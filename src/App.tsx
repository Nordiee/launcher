...
function Library({ accessToken }: { accessToken: string }) {
  ...
  return <>{offlineNotice}{installError && <p className="install-error" role="alert">{installError}</p>}<section className="library-grid" aria-label="Your game library">{games.map((game) => {
    ...
    return <article className="library-card" key={game.id}><div className="library-cover" aria-hidden="true">N</div><div><p className="panel-label">{isInstalled ? checkState === "repair" ? "REPAIR REQUIRED" : "INSTALLED" : isInstalling ? "DOWNLOADING" : game.installState}</p><h2>{game.title}</h2><p>{isInstalling && percentage !== null ? `${percentage}% downloaded` : checkState === "verified" ? "All installed files verified." : checkState === "repair" ? "One or more files need repair." : game.installSizeBytes ? `${Math.round(game.installSizeBytes / 1_000_000_000)} GB` : "Size will be available soon"}</p><div className="library-actions"><button className="library-action" type="button" disabled={isInstalling || checkState === "verifying"} onClick={() => void (isInstalled ? checkState === "repair" ? repair(game) : verify(game) : install(game))}>{isInstalled ? checkState === "verifying" ? "Verifying" : checkState === "repair" ? isInstalling ? percentage !== null ? `Repairing ${percentage}%` : "Preparing repair" : "Repair files" : "Verify files" : isInstalling ? percentage !== null ? `Downloading ${percentage}%` : "Preparing" : "Install"}</button>{isInstalled && <button className="library-action uninstall-action" type="button" disabled={isInstalling} onClick={() => void uninstall(game)}>Uninstall</button>}</div></div></article>;
  })}</section></>;
}
